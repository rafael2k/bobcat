...
#if defined (XCURSES)
	if (_cursvar.mono)
		sprintf(_display,"XWINDOWS-MONO-%dx%d",LINES,COLS);
	else
		sprintf(_display,"XWINDOWS-COLOR-%dx%d",LINES,COLS);
#endif

#ifdef OS2
# ifdef EMXVIDEO
	switch (_cursvar.mono)
	{
...
{
#if defined (XCURSES)
	chtype temp =   (A_NORMAL | A_BOLD); /* X cannot blink */
#else
