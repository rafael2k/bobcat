...
/*
 *	MSDos has no such define
 */
typedef unsigned long int ui_long;

typedef struct _connection {
    struct _connection *	next;	/* Link on list 	*/
    ui_long			addr;	/* IP address		*/
...
static void init_help_message_cache()
{
    if(help_message_buffer != NULL)
	free(help_message_buffer);
    help_message_buffer = NULL;
}
...
static char *help_message_cache_non_empty()
{
 if(help_message_buffer == NULL)
	return (0);
 else
...
static char *help_message_cache_contents()
{
...
#ifdef DT
    if (TRACE)
        fprintf(stderr, "FTP connected, status %d\n", status);
#endif
...
#ifdef DT
    if (TRACE) fprintf(stderr, "TCP: Master socket(), bind() and listen() all OK\n");
#endif
...
{
	short   i, j;
	auto unsigned long int    base=1L;
	auto unsigned long int size_num = 0UL;
	...
	/** Track down the size **/
	    ...
	    *cps = '\0';
	    ...
	    while ((cps=strtok(NULL, sp)) != NULL) {
	       ...
	       if (*cpd == '\0') {
		   /* Assume it's blocks */
		   entry_info->size = (unsigned long)atol(cps) * 512UL;
                   break;
               }
           }
	}
...
#ifdef DT
	if (TRACE) fprintf(stderr,
			 "HTFTP: VMS filename: %s  date: %s  size: %lu\n",
			 entry_info->filename,
  ...
  HTStream *,			sink )
{
    ...
    target = HTML_new(parent, format_out, sink);
...
#ifdef DT
        if (TRACE) fprintf(stderr, "TCP: Accepted new socket %d\n", status);
#endif
