...
*/
...

	{
	    ...
	    END(HTML_H1);
	    PUTS("\n");
	    PUTS("This is a link for searching this index.\n");
	...
	*/
	...
	START(HTML_P);
	PUTS("\n");
	PUTS("Enter the 's'earch command and then specify search words.\n");
	...
	for(count = 0; 
	  ...
	  if(0 ==
	      generate_retrieval_apdu(request_message + HEADER_LENGTH,
		    ...
		    FREE_TARGET;
		    if (type) free (type);
		    if (docid->bytes) free (docid->bytes);
		    goto CleanUp;
#else
...
*/
#ifdef VMS
    if (connection != NULL) NETCLOSE((int)connection);
#else
    if (connection)    close_connection(connection);
#endif /* VMS */
    if (wais_database) free(wais_database);
#ifdef VMS
    if (request_message)  free(request_message);
